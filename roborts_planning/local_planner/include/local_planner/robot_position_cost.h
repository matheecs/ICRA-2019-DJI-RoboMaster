...
class RobotPositionCost {
 public:
  ...
  /**
   * @brief Calculate the obstacle is which orientation of robot
   * @param x Robot current x
   * @param y Robot current y
   * @param theta Robot current theta
   * @param footprint_spec Robot vertices
   * @param check_size 扩展检查半径，以map分辨率增加size
   * @return -1:not in map or not recetangular 
   */
  int FootprintCostOrientation(double x, double y, double theta,
