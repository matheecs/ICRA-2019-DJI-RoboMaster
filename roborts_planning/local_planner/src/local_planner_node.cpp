...
void LocalPlannerNode::ExcuteCB(const roborts_msgs::LocalPlannerGoal::ConstPtr &command) {
  ...
  if (plan_mtx_.try_lock()) {
    ...
    local_planner_->SetMaxVel(command->maxvelx, command->maxvely);
    plan_mtx_.unlock();
...
void LocalPlannerNode::Loop() {
  ...
  while (GetNodeState() == NodeState::RUNNING) {
    ...
    if (error_info.IsOK()) {
      ...
      // if(error_count == max_error_-2 || error_count == max_error_)
      // {
      //   // cmd_vel_.twist.linear.x = -0.20;
      ...
      // }
      // else 
      if(error_count == max_error_-3 || error_count == max_error_-1){
        local_planner_->DirectComputeGlobalVelocity(cmd_vel_);
        vel_pub_.publish(cmd_vel_);
      }
      error_count++;
