 ...
 ***************************************************************************/
#ifndef ROBORTS_PLANNING_GLOBAL_PLANNER_LAZY_THETA_STAR_PLANNER_H
#define ROBORTS_PLANNING_GLOBAL_PLANNER_LAZY_THETA_STAR_PLANNER_H

#include <ros/ros.h>
#include <geometry_msgs/PoseStamped.h>

#include "proto/lazy_theta_star_planner_config.pb.h"
 ...
 */
class LazyThetaStarPlanner : public GlobalPlannerBase {
 ...
 public:
   ...
   */
  LazyThetaStarPlanner(CostmapPtr costmap_ptr);
  virtual ~LazyThetaStarPlanner();
  /**
 ...
 private:
  ...
  /**
   * @brief check if there has a line of sight or not.
   * @param index1 start point of costmap index
   * @param index2 end point of costmap index
   * @return Return true if there is a direct path between index1 and index2.
   */
  bool checkLineofSight(const int &index1, const int &index2);
  bool LargerDistance(const int &index1, const int &index2, const int &goal_index);
  ...
  struct Compare {
    bool operator()(const int &index1, const int &index2) {
      return LazyThetaStarPlanner::f_score_.at(index1) > LazyThetaStarPlanner::f_score_.at(index2);
    }
  };
  ...
  //! inaccessible_cost of lazy theta star algorithm threshold
  unsigned int lazy_theta_inaccessible_cost_;
  //!  largest distance threshold
  unsigned int largest_distance_up_;
  unsigned int largest_distance_down_;
  unsigned int goal_distance_;
  ...
  std::vector<LazyThetaStarPlanner::SearchState> state_;
  int cost_time_;
  float sam_thre_;
...
std::vector<int> LazyThetaStarPlanner::f_score_;
...
#endif // ROBORTS_PLANNING_GLOBAL_PLANNER_LAZY_THETA_STAR_PLANNER_H
