...
AStarPlanner::AStarPlanner(CostmapPtr costmap_ptr) :
  ...
  cut_time_ = a_star_planner_config.cut_time();
}
...
//    }
  ...
  // geometry_msgs::PoseStamped iter_pos_tmp;
  // int num = floor((path.size()-2) / cut_time_);
  // int last_num = path.size()-2-num*cut_time_;
  // path_tmp.push_back(path.front());
  // for(int i=0; i<num; i++){
  //   iter_pos_tmp.pose.position.x = 0;
  //   iter_pos_tmp.pose.position.y = 0;
  //   for(int j=1; j<=cut_time_; j++){
  //     iter_pos_tmp.pose.position.x += path[i*cut_time_+j].pose.position.x;
  //     iter_pos_tmp.pose.position.y += path[i*cut_time_+j].pose.position.y;
  //   }
  //   iter_pos_tmp.pose.position.x /= cut_time_;
  //   iter_pos_tmp.pose.position.y /= cut_time_;
  //   path_tmp.push_back(iter_pos_tmp);
  // }
  // if (last_num != 0){
  //   iter_pos_tmp.pose.position.x = 0;
  //   iter_pos_tmp.pose.position.y = 0;
  //   for(int j=1; j<=last_num; j++){
  //     iter_pos_tmp.pose.position.x += path[num*cut_time_+j].pose.position.x;
  //     iter_pos_tmp.pose.position.y += path[num*cut_time_+j].pose.position.y;
  //   }
  //   iter_pos_tmp.pose.position.x /= last_num;
  //   iter_pos_tmp.pose.position.y /= last_num;
  //   path_tmp.push_back(iter_pos_tmp);
  // }
  // path_tmp.push_back(path.back());
  // path = path_tmp;
